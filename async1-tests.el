;;; async1-tests.el --- Async chains of parallel and sequencial callbacks. -*- lexical-binding: t -*-
...
(ert-deftest test-async-plist-tests()
  ...
  (should
   (eql (async1-plist-get '(:parallel
                                 (:result "Parallel A" :delay 1)
                                 (:result "Parallel B" :delay 2)
                                 :aggregator #'async1-default-aggregator
                                 )
                               :aggregator)
  ...
  (should
   (eql (async1-plist-get '(:parallel
              ...
              nil))
  (should
   (equal (async1-plist-remove '(:parallel
                           :aggregator #'async1-default-aggregator
                                 (:result "Parallel A" :delay 1)
                                 (:result "Parallel B" :delay 2)
                                 )
...
(ert-deftest test-async-default-aggregator-multiple ()
  ...
  (reset-test-state)
  (advice-add 'print :around #'test-capture-print)
  (let ((result (async1-default-aggregator '("a" "b" "c"))))
...
(ert-deftest test-async-create-function-plist-defaults ()
  ...
  (reset-test-state)
  (let ((func (async1-create-function '())))
    (funcall func "input" (lambda (result) (setq test-results result))))
...
;;   (reset-test-state)
;;   (let ((func (async1-create-function 'custom-async-step)))
;;     (funcall func "input" (lambda (result) (setq test-results result)))
